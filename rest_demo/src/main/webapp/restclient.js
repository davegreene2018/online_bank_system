$(document).ready(function(){
  ...
  $(document).on('submit', '#login_customer', function(e) {
      ...
      request.done(function(output){
          var fullOutput = new Array(output);
          var customerList = fullOutput[0];
          
           for (var key in customerList) {
               if (customerList[key].email === email && customerList[key].password === password){
                   correctCustomer = customerList[key];
                   customerId = key+1;
                   window.location.href = 'main.html';
                   console.log('success');
               } else {
                   console.log('wrong customer/incorrect password email');
                   
               }
           }
           
           if(correctCustomer){
            console.log('Correct Customer' + correctCustomer);
            }else{
            window.alert('Email/password not valid.');
        }
  
          //redirect to new page with balance using cutomer id for everything.
      });
