$(document).ready(function(){
  ...
  // deposit
  // // Add customer button
  $(document).on('click', '#add_deposit', function(e){
    e.preventDefault();
    $('.lightbox_content h2').text('Deposit');
    $('#form_deposit button').text('Deposit');
    $('#form_deposit').attr('class', 'form add');
    $('#form_deposit').attr('data-id', '');
    $('#form_deposit .field_container label.error').hide();
    $('#form_deposit .field_container').removeClass('valid').removeClass('error');
    $('#form_deposit #transId').val('');
    $('#form_deposit #ammount').val('');
    show_lightbox();
  });

  $(document).on('submit', '#form_deposit.add', function(e){
    e.preventDefault();
    // Validate form
    if (form_deposit.valid() === true){
      hide_ipad_keyboard();
      hide_lightbox();
      show_loading_message();
      var form_data = $('#form_deposit').serializeJSON();
      var request   = $.ajax({
        url:          'api/transaction/',
        cache:        false,
        processData : false,
      ...
      request.done(function(output){
          $('#table_transactions').dataTable().api().ajax.reload(function(){
            hide_loading_message();
            var ammount_name = $('#name').val();
            show_message(ammount_name + " added successfully.", 'success');
          }, true);
      });
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    request.done(function(output){
      $('.lightbox_content h2').text('Deposit');
      $('#form_deposit button').text('Deposit');
      $('#form_deposit').attr('class', 'form edit');
      $('#form_deposit').attr('data-id', id);
      $('#form_deposit .field_container label.error').hide();
      $('#form_deposit .field_container').removeClass('valid').removeClass('error');
      $('#form_deposit #transId').val(output.transId);
      $('#form_deposit #postBalance').val(output.postBalance);
  ...
  // Edit customer submit form
  $(document).on('submit', '#form_deposit.edit', function(e){
    e.preventDefault();
    // Validate form
    if (form_deposit.valid() === true){
      // Post animal information
      ...
      hide_lightbox();
      show_loading_message();
      var id        = $('#form_deposit').attr('data-id');
      var form_data = $('#form_deposit').serializeJSON();
      var request   = $.ajax({
      ...
      request.done(function(output){
        ...
        $('#table_transactions').dataTable( ).api().ajax.reload(function(){
            hide_loading_message();
            var trans_name = $('#name').val();
            show_message("Customer '" + trans_name + "' successfully edited.", 'success');
        }, true);
      });
      request.fail(function(jqXHR, textStatus){
        hide_loading_message();
